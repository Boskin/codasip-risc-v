...
public:
  ...
  /// bundle will still be bundled after removing the single instruction.
  ///
  /// POSSIBLE BUG: what if MI is the HEAD of the bundle? if it deletes only the
  /// instruction, it is ok.
  /// if not, FIXME.
  ...
  // ======================== CODASIP ========================================
  /// same, but with supported bundles - the original doesn't take into account
  /// that terminator does not have to be last to be in the last bundle:
  void ReplaceUsesOfBlockWithSupportedBundles(MachineBasicBlock *Old, 
